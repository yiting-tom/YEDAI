...
|---|---|---|
| 完整 | 整段被圈成單一識別碼 | 正常 |
| **部分** | 被切成兩段，或只圈到一部分 | **視為失敗** |
| 未涵蓋 | 不符合任何樣式 | 視為失敗 |

「部分」特別要注意：被切成兩段的識別碼看起來「有被抓到」，但 `XTR-05#PM1` 變成
`ID:XTR05` + `ID:PM1` 之後，`ID:PM1` 在所有機台之間共用——那正是模式 A 才該有的病。
...
目前的索引格式版本是 **5**（v2 加入 `by_id` / `bundle_roots`，v3 加入關聯邊，
v4 讓 `#` 成為識別碼的一部分並加入父子層級展開，v5 加入 op no / lot / wafer / tech
樣式，並讓 `#` 的父層可含 `-`）。
